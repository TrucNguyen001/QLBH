<template>
  <div class="content">
    <div class="content-header">
      <div class="content-title">Quản lý mã giảm giá</div>
      <div>
          ...
          <tbody>
            ...
            >
              <td
                ...
                @click="selectedRecord(record.DiscountId, index)"
              >
                <input
              ...
              <td>{{ record.ReducedAmount }}</td>
              <td>{{ record.InputNumber }}</td>
              <td>
...
<script>
import DiscountDetail from "../discountcode/DiscountCodeDetail.vue";
export default {
  ...
  methods: {
    ...
    getRecordIdWhenMove(record) {
      this.recordId = record.DiscountId;
      this.recordCode = record.DiscountCode;
    ...
    async downloadFile() {
      ...
      try {
        ...
        await me.apiService.downloadFileExcel("Discount/ExportFile");
      ...
      try {
        ...
        if (Object.keys(this.listRecord).length !== 0) {
          ...
          for (let i = 0; i < me.listRecord.length; i++) {
            me.resultRecordIdList.push(me.listRecord[i].DiscountId);
          }
    ...
    async addRecord() {
      try {
        ...
        this.recordSelected = {};
        this.recordSelected.DiscountCode = this.recordCodeBiggest;
      } catch (error) {
        console.log(error);
      }
    },
    ...
    selectAllRecord: function (pageNumber) {
      try {
        if (this.listPageSelectAllRecord.includes(pageNumber)) {
          ...
          this.listRecordId = this.listRecordId.filter(
            (item) =>
              !this.listRecord.some((record) => record.DiscountId === item)
          );

          this.listIndexSelected = this.listIndexSelected.filter(
            (item) =>
              !this.listRecord.some((record) => record.DiscountId === item.key)
          );
        } else {
          ...
          this.listRecord.forEach((item) => {
            this.listRecordId.push(item.DiscountId);
            this.listRecordId = this.common.uniqueArray(this.listRecordId);
            let element = {};
            element.key = item.DiscountId;
            this.listIndexSelected.push(element);
          });
        }
      } catch (error) {
    ...
    async getRecordCodeBiggest() {
      try {
        ...
        if (this.listRecord == 0) {
          me.recordCodeBiggest = this.createRandomCode(8);
        } else {
          me.recordCodeBiggest = await me.apiService.get(
            "Discount/code-biggest"
          );
        }
      } catch (error) {
        console.log(error);
     ...
     * @param {độ dài mã} doDai
     * @author: Nguyễn Văn Trúc (6/4/2024)
     */
    createRandomCode(doDai) {
      var chuoi = "";
      var kyTu =
    ...
    checkRowSelected(record) {
      return (
        this.listIndexSelected.filter((i) => i.key === record.DiscountId)
          .length != 0
      );
    },
  },
